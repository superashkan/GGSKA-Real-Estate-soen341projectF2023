...
function PropertySearchForm() {
  ...
  const performSearch = function() {
    ...
    if (!isNullOrEmpty(maxPrice)) {
      resultsList = resultsList.filter(function(property) {
        return parseFloat(property.price) <= maxPrice;
      });
    }
    if (!isNullOrEmpty(minPrice)) {
      resultsList = resultsList.filter(function(property) {
        return parseFloat(property.price) >= minPrice;
      });
    }
    ...
    if (results.length == 0) {
      return (
        ...
        <form id="search-form">
          ...
          <input id="numBedrooms" name="numBedrooms" placeholder="# of bedrooms (1 to 5)" type="number" onInput={(event) => setBedrooms(event.target.value)} />
          <label htmlFor="numBathrooms"># of Bathrooms</label>
          <input id="numBathrooms" name="numBathrooms" placeholder="# of bathrooms (1 to 5)" type="number" onInput={(event) => setBathrooms(event.target.value)} />
              ...
              {results.map((result) => {
                return (
                  <tr>
                    ...
                    <td>${neatlyFormatValue(result.price)}</td>
                    <td>{result.type}</td>
                    <td>{neatlyFormatValue(result.lotSize)} sqft.</td>
